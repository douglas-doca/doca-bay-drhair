...
const TOOLS = [
  ...
  {
    name: "gestia_agendar",
    description: "Cria um agendamento no Gest. Usa idDoCliente (existente) OU idDoProspect (lead novo). A API valida grade do profissional e conflitos. ⚠️ ANTI-DUP: bloqueia chamadas idênticas em 5min.",
    inputSchema: {
      type: "object",
      properties: {
...
// ============================================
// 🆕 HELPER: Resolver salas permitidas por tenant
// ============================================
function getSalasPermitidas(tenantId: string | undefined): Set<number> | null {
  if (!tenantId) return null;
  try {
    const allClients = (clientService as any).clients;
    if (!allClients) return null;
    for (const [, client] of allClients) {
      const cfg: any = client?.config;
      if (cfg?.tenant_id === tenantId) {
        const salas = cfg?.gestia?.salasPermitidas;
        if (Array.isArray(salas) && salas.length > 0) {
          return new Set(salas.map((s: any) => s.id));
        }
        return null;
      }
    }
...
// ============================================
// 🆕 HELPER: Resolver client_id pelo tenant_id
// ============================================
function resolveClientId(tenantId: string | undefined): { clientId: string; resolved: boolean } {
  let clientId = "drhair-coronel"; // fallback histórico
  let resolved = false;
  if (!tenantId) return { clientId, resolved };
  try {
    const allClients = (clientService as any).clients;
    if (allClients) {
      for (const [id, client] of allClients) {
        if ((client?.config as any)?.tenant_id === tenantId) {
          clientId = id;
          resolved = true;
          break;
        }
...
async function handleGestiaTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  // 🆕 Auto-resolve unitId pelo tenant_id se não veio explicitamente.
  // O bot Claude passa tenant_id; o MCP resolve unitId do config gestia.unitId.
  let unitId = args.unitId as number;
  if (!unitId && args.tenant_id) {
    ...
    try {
      ...
      if (allClients) {
        for (const [, client] of allClients) {
          const cfg: any = client?.config;
          if (cfg?.tenant_id === tenantId && cfg?.gestia?.unitId) {
            unitId = cfg.gestia.unitId;
            logger.info(`Auto-resolved unitId=${unitId} from tenant_id=${tenantId}`, undefined, "GESTIA");
            break;
          }
        }
      }
    } catch (err: any) {
      logger.warn(`Failed to auto-resolve unitId: ${err.message}`, undefined, "GESTIA");
    }
  }
  ...
  switch (name) {
    ...
    case "gestia_combos_cadastro": {
      ...
      try {
        ...
        if (d.salas) {
          const permitidasCombo = getSalasPermitidas(args.tenant_id as string);
          const salasBase = permitidasCombo ? d.salas.filter((s: any) => permitidasCombo.has(s.id)) : d.salas;
          slim.salas = salasBase.map((s: any) => ({
            id: s.id,
            nome: s.nome,
          }));
        }
    ...
    case "gestia_consultar_agenda": {
      ...
      // Filtrar retorno pra reduzir tokens (API retorna objetos gigantes e às vezes ignora filtro de data)
      try {
        const items = (rawAgenda as any)?.data?.dadosDeHorarioAgendado || (rawAgenda as any)?.data || [];
        if (Array.isArray(items)) {
          // 🛡️ Filtro defensivo: GestIA às vezes ignora dataInicio/dataFim
          const dataInicioStr = args.dataInicio as string;
          const dataFimStr = args.dataFim as string;
          const itemsNoPeriodo = items.filter((a: any) => {
            const dataAg = (a.data || "").split("T")[0];
            return dataAg && dataAg >= dataInicioStr && dataAg <= dataFimStr;
          });
          // 🛡️ Cap absoluto pra evitar token overflow (Claude limit 200k)
          const MAX_ITEMS = 50;
          const itemsCapped = itemsNoPeriodo.slice(0, MAX_ITEMS);
          const foiCapado = itemsNoPeriodo.length > MAX_ITEMS;
          const foiFiltrado = itemsNoPeriodo.length < items.length;
          const slim = itemsCapped.map((a: any) => ({
            id: a.id,
          ...
          logger.info(`gestia_consultar_agenda: ${items.length} recebidos → ${itemsNoPeriodo.length} no período → ${slim.length} retornados${foiCapado ? " (CAPADO)" : ""}${foiFiltrado ? " (API ignorou filtro data)" : ""}`, undefined, "GESTIA");
          return {
            ok: true,
            data: slim,
            total: slim.length,
            total_no_periodo: itemsNoPeriodo.length,
            total_recebido_da_api: items.length,
            capado: foiCapado,
            api_ignorou_filtro_data: foiFiltrado,
            dataInicio: args.dataInicio,
            dataFim: args.dataFim,
          };
        }
      } catch (e: any) {
        logger.warn(`gestia_consultar_agenda: erro filtrando: ${e?.message}`, undefined, "GESTIA");
      }
      return rawAgenda;
    }
    ...
    case "gestia_agendar": {
      ...
      // ════════════════════════════════════════════════════════════
      // 🛡️ ANTI-DUPLICAÇÃO — bloqueia chamadas idênticas em 5min
      // ════════════════════════════════════════════════════════════
      if (args.tenant_id) {
        try {
          const cutoff = new Date(Date.now() - 5 * 60 * 1000).toISOString();
          const filterKey = args.idDoProspect
            ? `contexto_conversa->>id_prospect=eq.${args.idDoProspect}`
            : `contexto_conversa->>id_cliente=eq.${args.idDoCliente}`;
          const recentes = await supabaseService.request<any[]>("GET", "agendamentos", {
            query: `tenant_id=eq.${args.tenant_id}` +
                   `&data=eq.${args.data}` +
                   `&horario=eq.${args.horaInicial}:00` +
                   `&${filterKey}` +
                   `&created_at=gte.${cutoff}` +
                   `&select=id,contexto_conversa`,
          }) || [];

          if (recentes.length > 0) {
            const gid = recentes[0].contexto_conversa?.gestia_id;
            logger.warn(
              `🛡️ DUPLICATA BLOQUEADA: tenant=${args.tenant_id} ` +
              `prospect=${args.idDoProspect || "-"} cliente=${args.idDoCliente || "-"} ` +
              `data=${args.data} ${args.horaInicial} → gestia_id=${gid} (criado há <5min)`,
              undefined, "GESTIA"
            );
            return {
              ok: true,
              ...
              data: { id: gid },
              message: "Agendamento já existe — duplicação prevenida (anti-dup MCP)",
            };
          }
        } catch (e: any) {
          logger.warn(`Anti-dup check falhou (continuando): ${e?.message}`, undefined, "GESTIA");
        }
      }
      ...
      const gestResult = await gestiaClient.fetch("/agenda/inserir", { method: "POST", body, unitId });

      // ════════════════════════════════════════════════════════════
      // 🪞 DUAL-WRITE — shadow copy no Supabase com 3 fallbacks
      // ════════════════════════════════════════════════════════════
      try {
        const gr: any = gestResult;
        if (gr?.ok && args.tenant_id && !gr?.duplicate_blocked) {
          // Resolve client_id
          const { clientId, resolved: clientIdResolved } = resolveClientId(args.tenant_id as string);
          if (!clientIdResolved) {
            logger.error(
              `⚠️ client_id NÃO resolvido pra tenant=${args.tenant_id} — usando fallback "${clientId}". Verifique clientService.clients.`,
              undefined, "GESTIA"
            );
          }
          ...
          // ── TENTATIVA 1: lookup GestIA (pageSize aumentado de 200 → 1000) ──
          try {
            if (args.idDoProspect) {
              const pList: any = await gestiaClient.fetch(`/prospect?pageSize=1000`, { unitId });
              const items = pList?.data?.items || pList?.data || [];
              const p = Array.isArray(items) ? items.find((x: any) => x.id === args.idDoProspect) : null;
              if (p) {
                const tel = p.celular || p.telefones?.[0];
                if (tel) telefone = `${String(tel.ddd || "")}${String(tel.numero || "")}`.replace(/\D/g, "");
                nome = p.nomeCompleto || "";
                logger.info(`Prospect resolvido (GestIA): id=${p.id} nome=${nome} tel=${telefone}`, undefined, "GESTIA");
              } else {
                logger.warn(`Prospect ${args.idDoProspect} não encontrado na listagem GestIA (pageSize=1000)`, undefined, "GESTIA");
              }
            } else if (args.idDoCliente) {
              const cList: any = await gestiaClient.fetch(`/cliente?pageSize=1000`, { unitId });
              const items = cList?.data?.items || cList?.data || [];
              const c = Array.isArray(items) ? items.find((x: any) => x.id === args.idDoCliente) : null;
              if (c) {
                const tel = c.celular || c.telefones?.[0];
                if (tel) telefone = `${String(tel.ddd || "")}${String(tel.numero || "")}`.replace(/\D/g, "");
                nome = c.nomeCompleto || "";
                logger.info(`Cliente resolvido (GestIA): id=${c.id} nome=${nome} tel=${telefone}`, undefined, "GESTIA");
              } else {
                logger.warn(`Cliente ${args.idDoCliente} não encontrado na listagem GestIA (pageSize=1000)`, undefined, "GESTIA");
              }
            }
          } catch (lookupErr: any) {
            logger.warn(`Lookup GestIA falhou: ${lookupErr.message}`, undefined, "GESTIA");
          }

          // ── TENTATIVA 2: fallback via lead_id no Supabase ──
          if (!telefone && args.lead_id) {
            try {
              const lead = await supabaseService.getLeadById(args.tenant_id as string, args.lead_id as string);
              if (lead?.phone) {
                telefone = lead.phone.replace(/\D/g, "");
                nome = nome || lead.name || "";
                logger.info(`📱 Telefone via lead_id fallback: ${telefone} (lead=${args.lead_id})`, undefined, "GESTIA");
              }
            } catch (e: any) {
              logger.warn(`Fallback lead_id falhou: ${e?.message}`, undefined, "GESTIA");
            }
          }

          // ── TENTATIVA 3: fallback via ddd+numero dos args (se agente passou) ──
          if (!telefone && args.ddd && args.numero) {
            telefone = `55${args.ddd}${String(args.numero).replace(/\D/g, "")}`;
            logger.info(`📱 Telefone via args.ddd+numero: ${telefone}`, undefined, "GESTIA");
          }

          const gestiaAgendamentoId = gr?.data?.id || gr?.data?.data?.id;

          const baseBody = {
            tenant_id: args.tenant_id,
            client_id: clientId,
            data: args.data,
            horario: `${args.horaInicial}:00`,
            lead_id: (args.lead_id as string) || null,
            status: "aguardando_crm",
            confirmado_em: new Date().toISOString(),
            source: "gestia",
            contexto_conversa: {
              created_via: "ai_agent_gestia",
              gestia_id: gestiaAgendamentoId,
              gestia_unit_id: unitId,
              id_profissional: args.idDoProfissional,
              id_procedimento: args.idDoProcedimento,
              id_sala: args.idDaSala || null,
              id_prospect: args.idDoProspect || null,
              id_cliente: args.idDoCliente || null,
              observacao: args.observacao || null,
              hora_final: args.horaFinal,
              timestamp: new Date().toISOString(),
              needs_reconciliation: !telefone,
            },
          };

          try {
            if (!telefone) {
              // 🚨 Não pula mais silenciosamente — INSERT placeholder pra reconciliação posterior
              logger.error(
                `🚨 Shadow-write SEM TELEFONE: agendamento ${gestiaAgendamentoId} criado no Gest. ` +
                `tenant=${args.tenant_id} prospect=${args.idDoProspect} cliente=${args.idDoCliente} lead_id=${args.lead_id}. ` +
                `Inserindo placeholder pra reconciliação.`,
                undefined, "GESTIA"
              );
              await supabaseService.request("POST", "agendamentos", {
                body: { ...baseBody, telefone: `pending-gestia-${gestiaAgendamentoId}`, nome: nome || "[Pendente reconciliação]" },
              });
              logger.warn(`⚠️ Placeholder inserido — rodar reconciliação pra resolver telefone/nome`, undefined, "GESTIA");
            } else {
              await supabaseService.request("POST", "agendamentos", {
                body: { ...baseBody, telefone, nome: nome || null },
              });
              logger.info(`✅ Shadow-write OK: agendamento ${gestiaAgendamentoId} (${telefone})`, undefined, "GESTIA");
            }
          } catch (insErr: any) {
            if (String(insErr?.message || "").includes("duplicate") || String(insErr?.code) === "23505") {
              logger.warn(`Shadow-write duplicate (já existe): tenant=${args.tenant_id} data=${args.data} hora=${args.horaInicial}`, undefined, "GESTIA");
            } else {
              logger.error(`Shadow-write FALHOU: ${insErr?.message}`, insErr, "GESTIA");
            }
          }
        }
      } catch (mirrorErr: any) {
        logger.error(`Erro shadow-write pipeline: ${mirrorErr?.message}`, mirrorErr, "GESTIA");
      }
    ...
    case "gestia_alterar_situacao_agenda": {
      const alteraResult = await gestiaClient.fetch("/agenda/alterarsituacao", {
        method: "PUT",
        body: { id: args.idAgendamento, situacao: { id: args.idSituacao } },
        unitId,
      });
      // 🪞 Propaga status pro Supabase via gestia_id no contexto_conversa
      try {
        const ar: any = alteraResult;
        if (ar?.ok && args.tenant_id) {
          const sitMap: Record<number, { status: string; extra?: Record<string, any> }> = {
            1: { status: "aguardando_crm" },
            2: { status: "confirmado_crm", extra: { confirmado_em: new Date().toISOString() } },
            3: { status: "aguardando" },
            4: { status: "em_atendimento" },
            5: { status: "realizado", extra: { compareceu_em: new Date().toISOString() } },
          };
          const mapping = sitMap[args.idSituacao as number];
          if (mapping) {
            await supabaseService.request("PATCH", "agendamentos", {
              query: `tenant_id=eq.${args.tenant_id}&contexto_conversa->>gestia_id=eq.${args.idAgendamento}`,
              body: { status: mapping.status, ...(mapping.extra || {}) },
            });
            logger.info(`✅ Status shadow-sync: ${args.idAgendamento} → ${mapping.status}`, undefined, "GESTIA");
          }
        }
      } catch (syncErr: any) {
        logger.warn(`Shadow-sync status falhou: ${syncErr?.message}`, undefined, "GESTIA");
      }
      return alteraResult;
    }

    case "gestia_excluir_agendamento": {
      const delResult = await gestiaClient.fetch(`/agenda/${args.idAgendamento}`, { method: "DELETE", unitId });
      // 🪞 Marca cancelado no Supabase
      try {
        const dr: any = delResult;
        if (dr?.ok !== false && args.tenant_id) {
          await supabaseService.request("PATCH", "agendamentos", {
            query: `tenant_id=eq.${args.tenant_id}&contexto_conversa->>gestia_id=eq.${args.idAgendamento}`,
            body: { status: "cancelado", motivo_cancelamento: "Cancelado via GestIA" },
          });
          logger.info(`✅ Cancelamento shadow-sync: ${args.idAgendamento}`, undefined, "GESTIA");
        }
      } catch (syncErr: any) {
        logger.warn(`Shadow-sync cancelamento falhou: ${syncErr?.message}`, undefined, "GESTIA");
      }
      return delResult;
    }
    ...
    case "gestia_listar_salas": {
      const rawSalas: any = await gestiaClient.fetch("/sala", { unitId });
      // 🪞 Filtra salas permitidas (do config do tenant)
      const permitidas = getSalasPermitidas(args.tenant_id as string);
      if (permitidas && rawSalas?.data) {
        const items = Array.isArray(rawSalas.data?.items) ? rawSalas.data.items : (Array.isArray(rawSalas.data) ? rawSalas.data : null);
        if (items) {
          const filtradas = items.filter((s: any) => permitidas.has(s.id));
          logger.info(`Salas filtradas: ${filtradas.length}/${items.length} (permitidas: [${Array.from(permitidas).join(",")}])`, undefined, "GESTIA");
          if (Array.isArray(rawSalas.data?.items)) {
            rawSalas.data.items = filtradas;
            rawSalas.data.count = filtradas.length;
          } else {
            rawSalas.data = filtradas;
          }
        }
      }
      return rawSalas;
