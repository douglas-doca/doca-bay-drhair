...
const BOT_ACTIVE_CHATS = new Map<string, number>();
...
export function rememberBotSentId(id?: string | null, chatId?: string | null) {
  ...
  if (v) {
    ...
    if (v.includes("_")) BOT_SENT_IDS.set(v.split("_").pop()!, Date.now());
  }
  const chat = String(chatId || "").trim();
  if (chat) {
    BOT_ACTIVE_CHATS.set(chat, Date.now());
    const phone = chat.replace(/@c\.us$|@lid$|@g\.us$/, "");
    if (phone && phone !== chat) BOT_ACTIVE_CHATS.set(phone, Date.now());
  }
}
...
export function isBotSentId(id?: string | null, chatId?: string | null): boolean {
  ...
  if (v && v.includes("_") && BOT_SENT_IDS.has(v.split("_").pop()!)) return true;
  const chat = String(chatId || "").trim();
...
export class WAHAService {
  ...
  private resolveSession(override?: string): string {
  ...
  public async request(method: string, endpoint: string, body?: any): Promise<any> {
  ...
  private trackBotMessage(resp: any, chatId: string): void {
  ...
  async sendVoice(payload: SendVoicePayload): Promise<any> {
    const chatId = this.normalizeChatId(payload.chatId);
    if (!chatId || !payload.audioSource) return null;
  ...
  async sendVoiceBuffer(chatId: string, audioBuffer: Buffer, mimetype?: string, session?: string): Promise<any> {
  ...
  async sendMedia(payload: SendMediaPayload): Promise<any> {
    const chatId = this.normalizeChatId(payload.chatId);
    if (!chatId || !payload.mediaUrl) return null;
    const session = this.resolveSession(payload.session);
    const body: any = { session, chatId, file: { url: payload.mediaUrl } };
  ...
  async sendImage(chatId: string, imageUrl: string, caption?: string, session?: string): Promise<any> {
    return this.sendMedia({ chatId, mediaUrl: imageUrl, caption, session });
  }

  async sendFile(payload: SendFilePayload): Promise<any> {
    const chatId = this.normalizeChatId(payload.chatId);
    if (!chatId || !payload.fileSource) return null;
    const session = this.resolveSession(payload.session);
    ...
    const body: any = { session, chatId, file };
    if (payload.caption) body.caption = payload.caption;
    const resp = await this.request("POST", "/api/sendFile", body);
    this.trackBotMessage(resp, chatId);
    return resp;
  ...
  async downloadMedia(mediaUrl: string): Promise<Buffer> {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`WAHA download failed: ${res.status} ${res.statusText}`);
  ...
  async downloadMediaById(messageId: string, session?: string): Promise<Buffer> {
    ...
    try {
      const res = await fetch(url, { headers: { "X-Api-Key": this.config.apiKey }, signal: controller.signal });
      if (!res.ok) throw new Error(`WAHA download by ID failed: ${res.status}`);
      return Buffer.from(await res.arrayBuffer());
    } finally {
      clearTimeout(t);
    }
  ...
  async getMessages(chatId: string, limit: number = 20, session?: string): Promise<any> {
  ...
  async checkNumber(phone: string, session?: string): Promise<any> {
  ...
  async lidToPhone(lid: string, session?: string): Promise<string | null> {
    const s = this.resolveSession(session);
    try {
  ...
  async startTyping(chatId: string, session?: string): Promise<any> {
    const to = this.normalizeChatId(chatId);
    const s = this.resolveSession(session);
    if (!to) return null;
  ...
  async stopTyping(chatId: string, session?: string): Promise<any> {
    const to = this.normalizeChatId(chatId);
    const s = this.resolveSession(session);
    if (!to) return null;
  ...
  async sendTypingFor(chatId: string, ms: number, session?: string): Promise<void> {
    const to = this.normalizeChatId(chatId);
    const s = this.resolveSession(session);
    if (!to) return;
    try {
      ...
      try { await this.stopTyping(to, s); } catch {}
    }
  }

  // ─── MEDIA CONVERSION (WAHA Plus) ─────────────────

  async convertVoice(audioUrl: string, session?: string): Promise<Buffer> {
    const s = this.resolveSession(session);
    ...
    try {
      ...
      if (!res.ok) throw new Error(`WAHA voice convert failed: ${res.status}`);
      return Buffer.from(await res.arrayBuffer());
    } finally {
      clearTimeout(t);
    }
  ...
  async sendPlanV3(chatId: string, items: PlanItem[], session?: string): Promise<void> {
    ...
    for (const item of safeItems) {
      try {
        if (item.type === "typing") {
          if (item.action === "start") await this.startTyping(to, s);
          else await this.stopTyping(to, s);
          if (item.delayMs) await this.sleep(item.delayMs);
          continue;
        }
        if (item.type === "text") {
          const text = String(item.text || "").trim();
          if (text) await this.sendMessage({ chatId: to, text, session: s });
          if (item.delayMs) await this.sleep(item.delayMs);
          continue;
        }
      } catch (err) {
  ...
  getConfig(): Readonly<WAHAConfig> { return { ...this.config }; }
  getBaseUrl(): string { return this.config.baseUrl; }
  getApiKey(): string { return this.config.apiKey; }
  getSession(): string { return this.config.session; }
}
...
// ============================================
const tenantWahaInstances = new Map<string, WAHAService>();

export function getWahaForBaseUrl(baseUrl: string, apiKey?: string): WAHAService {
  if (!baseUrl) return wahaService;
...
// ============================================
const lidCache = new Map<string, { phone: string; ts: number }>();
const LID_CACHE_TTL = 24 * 60 * 60 * 1000; // 24h

// 💉 ALTERAÇÃO CHAVE AQUI: A função agora aceita a instância correta (wahaInstance)
export async function resolveLid(lid: string, session: string, wahaInstance: WAHAService = wahaService): Promise<string | null> {
