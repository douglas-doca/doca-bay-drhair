...
import { wahaService, zapiService } from "./index.js";
  ...
  private async sendHumanized(
    ...
    try {
      if (items?.length) {
        ...
        if (provider === "zapi") {
          await zapiService.sendPlanV3(chatId, items, tenantId);
          return;
      ...
      // fallback: 1 mensagem
      if (provider === "zapi") {
        await zapiService.sendMessage({ chatId, text: clean }, tenantId);
      } else {
