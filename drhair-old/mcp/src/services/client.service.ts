...
class ClientService {
  ...
  private instanceToClient: Map<string, string> = new Map();
  
  // 🆕 MAPAS DIRETOS PARA TENANT_ID (mais seguro)
  private instanceToTenant: Map<string, string> = new Map(); // instanceId -> tenant_id
  private phoneToTenant: Map<string, string> = new Map();    // telefone -> tenant_id
  private clientToTenant: Map<string, string> = new Map();   // clientId -> tenant_id
   ...
   */
  private loadSecretsFromEnv(id: string, config: any): any {
  ...
  private loadClient(id: string, clientPath: string): Client | null {
    ...
    if (fs.existsSync(secretsPath)) {
      try {
        ...
        if (secrets.zapi) config.zapi = { ...config.zapi, ...secrets.zapi };
        if (secrets.crm) config.crm = { ...config.crm, ...secrets.crm };
        if (secrets.waha) config.waha = { ...config.waha, ...secrets.waha };
   ...
   */
  detectTenant(opts: { phone?: string; instanceId?: string }): { tenantId: string; clientId: string } | null {
  ...
  getClientIdByTenant(tenantId: string): string | null {
    for (const [clientId, tid] of this.clientToTenant) {
      if (tid === tenantId) return clientId;
    }
    return null;
  }
