...
import { getLeadMemory } from "./services/memory.service.js";
...
async function handleToolCall(name, args) {
    ...
    const tenantId = args.tenant_id || getCurrentTenant();
    try {
        let result;
        switch (name) {
            // ── LEADS ────────────────────────────────
            case "buscar_lead": {
                if (args.lead_id) {
                    result = await supabaseService.getLeadById(tenantId, args.lead_id);
                }
                else if (args.telefone) {
                    result = await supabaseService.getLeadByPhone(tenantId, args.telefone);
                }
                else {
            ...
            case "atualizar_lead": {
                ...
                if (!leadId && args.telefone) {
                    const lead = await supabaseService.getLeadByPhone(tenantId, args.telefone);
                    if (lead) {
                        const updates = {};
                        if (args.nome)
                        ...
                        if (args.notas)
                            updates.notes = args.notas;
                        if (args.lead_stage)
                            updates.lead_stage = args.lead_stage;
            ...
            case "perfil_lead_completo": {
                let lead = null;
                if (args.lead_id) {
                    lead = await supabaseService.getLeadById(tenantId, args.lead_id);
                }
                else if (args.telefone) {
                    lead = await supabaseService.getLeadByPhone(tenantId, args.telefone);
                }
                if (!lead) {
                    result = { error: "Lead não encontrado" };
                    break;
                ...
                const [memories, health, conv] = await Promise.all([
                    getLeadMemory(tenantId, lead.id).catch(() => null),
                    emotionService.getLeadHealth(tenantId, lead.id).catch(() => null),
                    supabaseService.getConversationByPhone(tenantId, lead.phone).catch(() => null),
                ]);
                let recentMessages = [];
                if (conv) {
                    recentMessages = await supabaseService.getRecentMessages(tenantId, conv.id, 5).catch(() => []);
                }
                result = {
                    lead,
                    memories,
            ...
            case "historico_conversa": {
                let conv = null;
                if (args.telefone) {
                    conv = await supabaseService.getConversationByPhone(tenantId, args.telefone);
                }
                else if (args.lead_id) {
                    const lead = await supabaseService.getLeadById(tenantId, args.lead_id);
                    if (lead)
                        conv = await supabaseService.getConversationByPhone(tenantId, lead.phone);
                }
                if (!conv) {
                    result = { error: "Conversa não encontrada" };
                    break;
                }
                const limit = args.limit || 20;
            ...
            case "registrar_mensagem": {
                ...
                if (!convId) {
                    result = { error: "conversation_id obrigatório" };
                    break;
                }
            ...
            case "conversas_pendentes": {
                const limit = args.limit || 20;
                result = await supabaseService.getConversationsByStatus(tenantId, "open");
            ...
            case "buscar_conversa_lead": {
                if (!args.telefone) {
                    result = { error: "telefone obrigatório" };
                    break;
                }
                result = await supabaseService.getConversationByPhone(tenantId, args.telefone);
                break;
            }
            // ── MEMÓRIA ──────────────────────────────
            ...
            case "salvar_memoria": {
                ...
                const memoryData = {
                    tenant_id: tenantId,
                    lead_id: leadId,
                    type: ({ 'fato': 'fact', 'contexto': 'semantic', 'preferencia': 'preference', 'objecao': 'objection', 'compromisso': 'commitment', 'episodico': 'episodic' }[args.tipo?.toLowerCase()] || args.tipo || 'fact'),
                    content: args.conteudo,
                    created_at: new Date().toISOString(),
                };
                try {
                    await supabaseService.request("POST", "lead_memories", { body: memoryData });
                    result = { success: true, message: "Memória salva", tenant_id: tenantId, lead_id: leadId, type: memoryData.type };
                }
                catch (memErr) {
                    logger.error("Failed to save memory", { error: memErr?.message, type: memoryData.type, lead_id: leadId }, "MCP");
                    result = { error: "Falha ao salvar memória", detail: memErr?.message };
                }
                break;
            }
            case "melhor_estrategia": {
                ...
                const [memories, health] = await Promise.all([
                    getLeadMemory(tenantId, leadId).catch(() => null),
                    emotionService.getLeadHealth(tenantId, leadId).catch(() => null),
                ]);
                const objections = Array.isArray(memories) ? memories.filter((m) => m.type === "objection") : [];
                const commitments = Array.isArray(memories) ? memories.filter((m) => m.type === "commitment") : [];
                result = {
                    lead_id: leadId,
                    health_score: health,
                    objections_conhecidas: objections.map((o) => o.content),
                    compromissos: commitments.map((c) => c.content),
            ...
            case "registrar_emocao": {
                await emotionService.saveEmotionEvent(tenantId, {
                    ...
                    source: "agent",
                    message_content: args.trigger || "conversation",
                });
                result = { success: true, message: "Emoção registrada" };
                break;
            }
            case "tendencia_emocional": {
                const leadHealth = await emotionService.getLeadHealth(tenantId, args.lead_id);
                if (!leadHealth) {
                    result = { error: "Sem dados emocionais para este lead" };
                    break;
                }
                result = {
                    lead_id: args.lead_id,
            ...
            case "sugerir_abordagem": {
                const lHealth = await emotionService.getLeadHealth(tenantId, args.lead_id);
                if (!lHealth) {
                    result = { sugestao: "Sem dados emocionais. Use abordagem neutra e acolhedora." };
                    break;
                }
            ...
            case "buscar_conhecimento": {
                ...
                try {
                    const kbResults = await supabaseService.request("GET", "knowledge_base", {
                    ...
                    result = kbResults && kbResults.length > 0
                        ? kbResults.map((kb) => ({ id: kb.id, category: kb.category, question: kb.question, answer: kb.answer }))
            ...
            case "buscar_provas_sociais": {
                try {
                    let q = `tenant_id=eq.${tenantId}&active=eq.true&order=priority.desc&limit=${args.limit || 3}`;
                    if (args.regiao) {
                        q += `&tags=cs.{${encodeURIComponent(args.regiao)}}`;
                    }
                    const proofs = await supabaseService.request("GET", "social_proof_assets", { query: q });
            ...
            case "buscar_agendamentos": {
                try {
                    ...
                    if (args.lead_id)
                        q += `&lead_id=eq.${args.lead_id}`;
                    if (args.telefone)
                        q += `&telefone=eq.${args.telefone}`;
                    if (args.data)
                        q += `&data=eq.${args.data}`;
                    q += "&order=data.desc,horario.desc&limit=10";
                    result = await supabaseService.request("GET", "agendamentos", { query: q });
                }
                catch {
            ...
            case "atualizar_agendamento": {
                try {
                    ...
                    result = await schedulerService.atualizarAgendamento(args.telefone || "", args.nova_data || "", args.novo_horario || "", {
                        nome: args.nome,
                        dataNascimento: args.data_nascimento,
                    });
                }
            ...
            case "waha_send_message":
                result = await wahaService.sendMessage({ chatId: args.phone, text: args.message });
                break;
            ...
            case "analysis_get_stalled_conversations":
                result = await analysisService.getStalledConversations({
                    tenantId, min_minutes: args.min_minutes ?? 240, limit: args.limit ?? 20, status: args.status ?? "open",
                });
                break;
            case "analysis_get_summary":
                result = await analysisService.getSummary({ tenantId, range: args.range ?? "today" });
                break;
            case "analysis_run_followup":
                result = await analysisService.runAnalysis({
                    tenantId, conversation_id: args.conversation_id, mode: args.mode ?? "followup", language: args.language ?? "pt-BR",
                });
                break;
            case "analysis_approve_send_followup":
                result = await analysisService.approveAndSend({
                    tenantId, conversation_id: args.conversation_id, text: args.text, followup_id: args.followup_id || undefined, phone: args.phone || undefined,
                });
                break;
            // ── EMOTION DASHBOARD ────────────────────
            ...
            case "emotion_get_lead_health":
                result = await emotionService.getLeadHealth(tenantId, args.lead_id);
                break;
            // ── UTILIDADE ────────────────────────────
...
async function handleReadResource(uri) {
    ...
    switch (uri) {
        ...
        case "doca://status/waha":
            try {
                const status = wahaService.getSessionStatus ? await wahaService.getSessionStatus() : { status: "unknown" };
...
// HTTP + SSE Server
// ============================================
async function handleSseConnection(req, res, transports) {
...
async function handleMessagePost(req, res, transports) {
        ...
        const httpServer = createServer(async (req, res) => {
            try {
                ...
                if (req.url?.startsWith("/message") && req.method === "POST") {
                    await handleMessagePost(req, res, transports);
                    return;
                }
                if (req.url === "/health") {
                    res.writeHead(200);
                    res.end(JSON.stringify({ status: "ok", tools: TOOLS.length, version: SERVER_VERSION }));
                    return;
                }
                res.writeHead(404);
                res.end("Not found");
