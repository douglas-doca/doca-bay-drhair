 ...
 */
function normalizarRespostaScheduler(raw) {
    if (!raw || !raw.success)
        return raw;
    // Já tem salas + horarios: considera canônico
    if (raw.salas && raw.horarios)
        return raw;
...
class SchedulerService {
     ...
     */
    async resolveSchedulerUrl(clientId, tenantId) {
        // 1. Config local
        const config = clientService.getClientConfig(clientId);
        const localUrl = config?.scheduler_url;
        if (localUrl)
            return localUrl;
        // 2. Supabase
        if (tenantId) {
            try {
                ...
                const url = result?.[0]?.scheduler_url;
                if (url)
                    return url;
            }
            catch { }
    ...
    getSchedulerAuth(clientId) {
        const config = clientService.getClientConfig(clientId);
        if (!config)
            return null;
        const schedulerAuth = config.scheduler_auth;
        if (schedulerAuth)
    ...
    // PARSING DE DATAS
    // ============================================
    parseDataRelativa(texto, dataAtual) {
    ...
    detectSchedulingIntent(message) {
        ...
        let periodoPreferido = null;
        if (/\b(manha|manhã|cedo|9|10|11)\s*(h|hrs|horas)?\b/.test(msg))
            periodoPreferido = 'manha';
        else if (/\b(tarde|13|14|15|16|17)\s*(h|hrs|horas)?\b/.test(msg))
            periodoPreferido = 'tarde';
        else if (/\b(noite|18|19|20)\s*(h|hrs|horas)?\b/.test(msg))
        ...
        try {
            ...
            if (resolvedTenantId) {
                ...
                if (cacheResult && cacheResult.horarios && cacheResult.horarios.length > 0) {
                    // Filtrar por duração da avaliação no cache também
                    const duracaoCache = 60;
                    const slotsCache = duracaoCache / 10;
                    const cacheHorarios = cacheResult.horarios;
                    cacheResult.horarios = cacheHorarios.filter((h) => {
                        const [hr, min] = h.split(':').map(Number);
                        const inicioMin = hr * 60 + min;
                        for (let i = 1; i < slotsCache; i++) {
                            const nextMin = inicioMin + (i * 10);
                            const nextH = String(Math.floor(nextMin / 60)).padStart(2, '0') + ':' + String(nextMin % 60).padStart(2, '0');
                        }
                        return true;
                    });
                    logger.info("✅ Horários do CACHE (filtrado duração)", { total: cacheResult.horarios.length, original: cacheHorarios.length }, "SCHEDULER");
    ...
    // ============================================
    async consultarScheduler(clientId, dataISO, tenantId) {
    ...
    // SALAS EM MEMÓRIA
    // ============================================
    salvarSalasEmMemoria(tenantId, dataISO, salas) {
        salasCache.set(`${tenantId}:${dataISO}`, { data: dataISO, salas, timestamp: Date.now() });
    }
    recuperarSalasDeMemoria(tenantId, dataISO) {
        const cached = salasCache.get(`${tenantId}:${dataISO}`);
        if (!cached)
            return null;
        if (Date.now() - cached.timestamp > CACHE_TTL_MS) {
            salasCache.delete(`${tenantId}:${dataISO}`);
            return null;
        }
        return cached.salas;
    }
    async encontrarSalaPorHorario(clientId, dataISO, horario, tenantId) {
        ...
        if (salas) {
            for (const [salaId, info] of Object.entries(salas)) {
                if (info.horarios?.includes(horario))
                    return { salaId, nome: info.nome, profissional: info.profissional };
            }
        }
        // 2. Cache Supabase
        try {
            ...
            if (cache?.salas) {
                for (const [salaId, info] of Object.entries(cache.salas)) {
                    if (info.horarios?.includes(horario))
                        return { salaId, nome: info.nome, profissional: info.profissional };
                }
            }
    ...
    // CACHE SUPABASE
    // ============================================
    async consultarCache(tenantId, dataISO) {
        try {
            const result = await supabaseService.request("GET", "horarios_cache", {
            ...
            const horarios = (cache.horarios_disponiveis || []).filter((h) => !bloqueados.includes(h));
    ...
    // ============================================
    async getHorariosBloqueados(tenantId, dataISO) {
        try {
    ...
    // CRIAR AGENDAMENTO
    // ============================================
    async criarAgendamento(clientId, dados, tenantId) {
        const tid = tenantId || this.getTenantId(clientId);
        if (!tid)
        ...
        try {
            ...
            if (result && !result.error) {
                const agendamentoId = Array.isArray(result) ? result[0]?.id : result?.id;
            ...
            if (error.message?.includes('unique') || error.code === '23505') {
                return { success: true, message: "Agendamento já existe (duplicate key). Horário reservado!", duplicate: true };
            }
            return { success: false, error: error.message };
        }
    }
    ...
    // ============================================
    async atualizarAgendamento(telefone, data, horario, dados) {
        try {
            let dataISO = data;
            if (!/^\d{4}-\d{2}-\d{2}$/.test(data))
                dataISO = this.parseDataRelativa(data);
            ...
            if (dataNascISO)
                updates.data_nascimento = dataNascISO;
    ...
    // ============================================
    formatHorariosParaPrompt(horarios, data, periodoSolicitado, salas) {
    ...
    formatErroConsultaParaPrompt(data, erro) {
        const dataFormatada = this.formatarDataExibicao(data);
    ...
    // HELPERS
    // ============================================
    filtrarPorPeriodo(horarios, periodo) {
    ...
    selecionarSugestoes(horarios, qtd) {
        if (horarios.length <= qtd)
            return horarios;
        const result = [];
        const step = Math.floor(horarios.length / qtd);
        for (let i = 0; i < qtd && i * step < horarios.length; i++)
            result.push(horarios[i * step]);
        return result;
    }
}
