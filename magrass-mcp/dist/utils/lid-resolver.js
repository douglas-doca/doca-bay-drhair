 ...
 */
export async function resolveLidPhone(lid, tenantId, wahaSession) {
    if (!lid || lid.length < 5)
        return null;
    // 1. Cache em memória
    ...
    // 2. Tabela lid_phone_map no Supabase
    try {
        const mapResult = await supabaseService.request("GET", "lid_phone_map", { query: `lid=eq.${lid}&limit=1` });
    ...
    try {
        ...
        const convResult = await supabaseService.request("GET", "conversations", { query: `chat_id=eq.${lid}${tenantFilter}&limit=1` });
...
export function saveLidMapping(lid, phone, tenantId) {
...
export function registerLidPhone(lid, phone, tenantId) {
    if (!lid || !phone)
        return;
    lidPhoneCache.set(lid, { phone, ts: Date.now() });
    saveLidMapping(lid, phone, tenantId);
...
export function stripSuffix(id) {
    return String(id || "").replace(/@(c\.us|g\.us|lid)$/g, "").trim();
}
//# sourceMappingURL=lid-resolver.js.map