...
class UnObjectScraper {
  constructor(config) {
    this.config = {
      ...
      // ✅ FIX: Sem hardcode — pega primeira sala do config
      sala: config.sala || (config.salas?.[0]?.nome || 'Sala 1'),
      salas: config.salas || [],
  ...
  // =============================================================================
  // 🔥 Selecionar múltiplas salas de uma vez
  // =============================================================================
  ...
  // =============================================================================
  // Método original mantido para compatibilidade (single sala)
  // =============================================================================
  ...
  // =============================================================================
  // Sessão pronta COM seleção de sala (para métodos single-sala)
  // =============================================================================
  ...
  async garantirSessaoPronta() {
    ...
    if (this.isLoggedIn && this.page) {
      try {
        ...
        if (currentUrl.includes('schedule') && !currentUrl.includes('login')) {
          this.log('Sessão já está ativa', 'info');
          this.log('Já está na agenda', 'info');
          return;
        }
        ...
        this.log('Sessão expirou, refazendo login...', 'warning');
      }
      this.isLoggedIn = false;
    }
    await this.login();
  ...
  // =============================================================================
  // ✅ FIX: Sessão pronta SEM seleção de sala (para multi-sala)
  // selecionarTodasAsSalas faz a seleção depois
  // =============================================================================

  async garantirSessaoProntaSemSala() {
    await this.init();
    if (this.isLoggedIn && this.page) {
      try {
        const currentUrl = this.page.url();
    ...
    await this.login();
    await this.irParaAgenda();
    // NÃO seleciona sala — selecionarTodasAsSalas faz isso
  }
  ...
  // =============================================================================
  // Extrair horários por coluna de sala
  // =============================================================================
  
  async extrairHorariosPorSala(salas) {
    this.log('🔍 Extraindo horários por sala...', 'debug');
    ...
    const resultado = await this.page.evaluate((salasConfig) => {
      ...
      colunas.forEach((coluna, colunaIdx) => {
        ...
        const headerNessaPosicao = headers[colunaIdx];
        ...
        eventos.forEach(evento => {
          ...
          if (label) {
            ...
            if (match) {
              ...
              agendamentos.push({ horarioInicio, horarioFim });
        ...
        if (todosHorariosSet.size < 10) {
          console.log(`[DEBUG] ⚠️ [EXTRAÇÃO] Poucos horários extraídos (${todosHorariosSet.size}), gerando grade completa...`);
        ...
        console.log(`[DEBUG] ✅ [EXTRAÇÃO] ${salaInfo.nome}: ${agendamentos.length} agendamentos, ${todosHorariosCompletos.length} horários`);
      });
  ...
  // =============================================================================
  // ✅ FIX: Consultar horários multi-sala — usa garantirSessaoProntaSemSala
  // =============================================================================
  ...
  async consultarHorariosMultiSala(dataStr, salas) {
    ...
    try {
      // ✅ FIX: Não seleciona sala individual antes — selecionarTodasAsSalas faz isso
      await this.garantirSessaoProntaSemSala();
      ...
      // ✅ Selecionar TODAS as salas de uma vez (antes era feito no garantirSessaoPronta)
  ...
  async criarAgendamento(dados) {
    ...
    try {
        ...
        for (const el of allElements) {
          ...
          if (text === horario && el.offsetWidth < 100 && el.offsetHeight < 50) {
            ...
            if (targetElement) {
              ...
              return { success: true, coords: { x, y }, targetTag: targetElement.tagName, targetClass: targetElement.className };
            }
          }
        }
        return { success: false };
        ...
        modalAberto = await this.page.evaluate(() => {
          const modal = document.querySelector('div[role="dialog"]') ||
                        ...
                        Array.from(document.querySelectorAll('*')).find(el => {
                          const text = el.textContent || '';
                          const rect = el.getBoundingClientRect();
                          return text.includes('Agendamento') && text.includes('Cliente') && rect.width > 400 && rect.height > 300;
                        });
          return !!modal;
        });
        if (modalAberto) { this.log('✅ Modal detectado!', 'success'); break; }
      ...
      await this.page.evaluate(() => {
        ...
        if (campo) { campo.value = '+55'; campo.dispatchEvent(new Event('input', { bubbles: true })); }
      });
      await this.sleep(500);
      ...
      await this.page.evaluate(() => { const campo = document.getElementById('name'); if (campo) campo.value = ''; });
      await this.sleep(300);
      ...
      await this.sleep(800);
      await this.page.evaluate(() => { const label = document.querySelector('label[for="cellPhone"]'); if (label) label.click(); });
      await this.sleep(1000);
      ...
      const campoDataEncontrado = await this.page.evaluate(() => {
        ...
        if (campoData) {
          ...
          return { found: true, id: campoData.id, name: campoData.name };
        }
        return { found: false };
      });

      if (campoDataEncontrado.found) {
        const seletor = campoDataEncontrado.id ? `#${campoDataEncontrado.id}` : `input[name="${campoDataEncontrado.name}"]`;
        await this.sleep(500);
        await this.page.click(seletor);
        await this.sleep(800);
        await this.page.evaluate((sel) => { const campo = document.querySelector(sel); if (campo) campo.value = ''; }, seletor);
        await this.sleep(300);
        ...
        await this.page.evaluate((sel, data) => {
          ...
          if (campo) {
            ...
            campo.removeAttribute('required');
            campo.setCustomValidity('');
            ['input', 'change', 'blur'].forEach(tipo => { campo.dispatchEvent(new Event(tipo, { bubbles: true })); });
          }
      ...
      try {
            ...
            if (path) {
              ...
              while (parent && attempts < 5) {
                ...
                if (text.includes('Gênero')) {
                  svg.dispatchEvent(new MouseEvent('click', { view: window, bubbles: true, cancelable: true }));
        ...
        if (dropdownAberto) this.log('✅ Dropdown de gênero aberto', 'success');
        await this.sleep(2000);
        ...
        const masculinoSelecionado = await this.page.evaluate(() => {
          ...
          if (opcoes.length === 0) opcoes = document.querySelectorAll('ul[role="listbox"] li, ul li');
          for (const opcao of opcoes) {
            const texto = opcao.textContent?.trim();
            if (texto && texto.toLowerCase() === 'masculino') {
              opcao.click();
              opcao.dispatchEvent(new MouseEvent('click', { view: window, bubbles: true, cancelable: true }));
              return true;
            }
          }
          return false;
        ...
        for (const el of elementos) {
          if (el.textContent.includes('Serviço') && el.textContent.length < 50) {
            ...
            if (parent) {
              ...
              if (dropdown && dropdown.offsetParent !== null) { dropdown.click(); return true; }
            }
          }
        }
        return false;
      ...
      if (voltou) {
        ...
        return { success: true, message: 'Agendamento criado com sucesso!', dados };
      } else {
        this.log('⚠️ Modal ainda aberto, mas agendamento pode ter sido criado', 'warning');
        return { success: true, message: 'Agendamento provavelmente criado', dados };
      }
  ...
  // =============================================================================
  // Criar agendamento em sala específica
  // =============================================================================
